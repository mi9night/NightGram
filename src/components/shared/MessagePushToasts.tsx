...
export function MessagePushToasts() {
  ...
  const enabled = user?.notificationSettings?.messages !== false;
  ...
  useEffect(() => {
    ...
    const onPush = ({
      ...
      if (isActiveChat || !enabled || muted) return;
      const body = message.text || (message.attachmentUrl ? "Медиафайл" : "Новое сообщение");
      const item: PushToast = {
        id: `${conversationId}:${message.id}:${Date.now()}`,
        conversationId,
        title: conversationTitle || "Новое сообщение",
        body,
  ...
  }, [activeConversationId, enabled, status, user?.id]);
    ...
    <div className="fixed bottom-5 right-4 z-[10085] flex w-80 max-w-[calc(100vw-2rem)] flex-col gap-2 pointer-events-none">
      <AnimatePresence initial={false}>
        {visible.map((item) => (
          ...
          >
            <GlowAvatar src={item.avatarUrl} alt={item.title} size={40} glow="purple" />
            <div className="min-w-0 flex-1">
              <div className="font-semibold text-sm truncate flex items-center gap-1.5">
                <MessageCircle size={13} className="text-neon-purple" /> {item.title}
              </div>
