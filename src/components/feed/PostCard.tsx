...
import type { MediaViewerItem } from "@/components/shared/MediaViewer";
...
const CommentSheet = dynamic(
  () => import("./CommentSheet").then((module) => module.CommentSheet),
  { ssr: false, loading: () => <div className="p-4 text-center text-sm text-white/40">Загрузка комментариев…</div> },
);

const MediaViewer = dynamic(
  () => import("@/components/shared/MediaViewer").then((module) => module.MediaViewer),
  { ssr: false },
);
...
export const PostCard = memo(function PostCard({
  ...
  const likedRef = useRef(post.liked);
  const confirmedLikedRef = useRef(post.liked);
  const confirmedLikesCountRef = useRef(post.likesCount);
  const likeSyncingRef = useRef(false);
  const savedRef = useRef(post.saved);
  const confirmedSavedRef = useRef(post.saved);
  const saveSyncingRef = useRef(false);

  useEffect(() => {
    likedRef.current = post.liked;
    confirmedLikedRef.current = post.liked;
    confirmedLikesCountRef.current = post.likesCount;
    setLiked(post.liked);
    setLikes(post.likesCount);
  }, [post.id, post.liked, post.likesCount]);

  useEffect(() => {
    savedRef.current = post.saved;
    confirmedSavedRef.current = post.saved;
    setSaved(post.saved);
  }, [post.id, post.saved]);
  ...
  async function syncLikeState() {
    ...
    try {
      while (confirmedLikedRef.current !== likedRef.current) {
        const response = await api.toggleLike(post.id);
        confirmedLikedRef.current = response.liked;
        confirmedLikesCountRef.current = response.likesCount;
        setLikes(response.likesCount);
        if (confirmedLikedRef.current === likedRef.current) {
          setLiked(response.liked);
        }
      }
    } catch {
      likedRef.current = confirmedLikedRef.current;
      setLiked(confirmedLikedRef.current);
      setLikes(confirmedLikesCountRef.current);
      showToast("Не удалось синхронизировать лайк", "error");
    } finally {
      likeSyncingRef.current = false;
      if (confirmedLikedRef.current !== likedRef.current) void syncLikeState();
    }
  }
  ...
  async function syncSaveState() {
    if (saveSyncingRef.current) return;
    saveSyncingRef.current = true;
    try {
      while (confirmedSavedRef.current !== savedRef.current) {
        const response = await api.toggleSave(post.id);
        confirmedSavedRef.current = response.saved;
        if (confirmedSavedRef.current === savedRef.current) setSaved(response.saved);
      }
    } catch {
      savedRef.current = confirmedSavedRef.current;
      setSaved(confirmedSavedRef.current);
      persistSavedState(confirmedSavedRef.current);
      showToast("Не удалось синхронизировать сохранение", "error");
    } finally {
      saveSyncingRef.current = false;
      if (confirmedSavedRef.current !== savedRef.current) void syncSaveState();
    }
  }
  ...
  return (
    ...
    >
      ...
      <AnimatePresence initial={false}>
        {onCommentOpen && (
          <CommentSheet postId={post.id} commentsEnabled={authorChannel?.commentsEnabled !== false} canModerate={Boolean(authorChannel && ["owner", "co_owner", "admin", "moderator"].includes(authorChannel.myRole ?? ""))} onClose={() => setOnCommentOpen(false)} />
        )}
      </AnimatePresence>
