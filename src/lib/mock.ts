...
import { DEFAULT_NOTIFICATION_SETTINGS as FULL_NOTIFICATION_SETTINGS } from "./notificationPreferences";
import { uid } from "./utils";

const now = Date.now();
const mins = (m: number) => new Date(now - m * 60_000).toISOString();

export const DEFAULT_NOTIFICATION_SETTINGS: NotificationSettings = { ...FULL_NOTIFICATION_SETTINGS };
