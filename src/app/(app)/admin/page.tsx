...
function FinanceSection() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const data = await Promise.race([api.getPurchaseRequests(filter), new Promise((_, r) => setTimeout(() => r(new Error('timeout')), 3000))]); setRequests(data as Record<string, unknown>[]); } catch { setRequests([]); }
    setLoading(false);
  }, [filter]);
