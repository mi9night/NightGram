...
router.get("/:username", async (req, res) => {
  ...
  if (error || !data) {
    ...
    let { data: byId } = await supabase
      ...
      .maybeSingle();
    if (!byId && /^[0-9a-f-]{32,36}$/i.test(req.params.username)) {
      const byUuid = await supabase
        .from("users")
        .select("*")
        .eq("id", req.params.username)
        .maybeSingle();
      byId = byUuid.data;
    }
    if (!byId) return res.status(404).json({ error: "User not found" });
    const { data: presence } = await supabase.from("presence").select("is_online,last_seen").eq("user_id", byId.id).maybeSingle();
    const { password_hash, ...safe } = byId;
    return res.json({ ...safe, is_online: Boolean(presence?.is_online && (!presence?.last_seen || Date.now() - new Date(presence.last_seen).getTime() < 90_000)), last_seen: presence?.last_seen || null });
  }
  const { data: presence } = await supabase.from("presence").select("is_online,last_seen").eq("user_id", data.id).maybeSingle();
  const { password_hash, ...safe } = data;
  res.json({ ...safe, is_online: Boolean(presence?.is_online && (!presence?.last_seen || Date.now() - new Date(presence.last_seen).getTime() < 90_000)), last_seen: presence?.last_seen || null });
});
...
router.patch("/me", async (req, res) => {
  ...
  let result = await supabase
    ...
    .select("*")
    .single();
  if (result.error && /night_status|music_|room_scene|hide_purchases|schema cache/i.test(result.error.message || "")) {
