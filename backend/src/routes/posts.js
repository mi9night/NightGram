...
router.post("/:id/comments", async (req, res) => {
  ...
  if (result.error) return res.status(500).json({ error: result.error.message });
  const { data: post } = await supabase.from("posts").select("comments_count,author_user_id").eq("id", req.params.id).single();
...
router.post("/comments/:id/pin", async (req, res) => {
  const { id } = req.params;
  const { data: comment } = await safe(supabase.from("comments").select("id,author_id,pinned").eq("id", id).maybeSingle(), { data: null });
  if (!comment) return res.status(404).json({ error: "Комментарий не найден" });
  const { data: me } = await safe(supabase.from("users").select("role").eq("id", req.userId).maybeSingle(), { data: null });
  const isAdmin = ["admin", "owner", "co_owner", "moderator"].includes(me?.role || "");
  if (comment.author_id !== req.userId && !isAdmin) return res.status(403).json({ error: "Нет прав на закрепление" });
...
router.delete("/comments/:id", async (req, res) => {
  ...
  const { data: fullComment } = await supabase.from("comments").select("post_id").eq("id", id).single();
