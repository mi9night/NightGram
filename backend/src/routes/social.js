...
const { blockState } = require("../lib/privacy");
...
router.post("/friend", async (req, res) => {
  ...
  if (blocks.blocked) return res.status(403).json({ error: "blocked", message: "Подписка недоступна из-за чёрного списка" });
...
router.post("/block", async (req, res) => {
  ...
  if (!userId || userId === req.userId) return res.status(400).json({ error: "Invalid user" });
  const current = await blockState(req.userId, userId);
  if (current.blockedByA) {
    await safe(supabase.from("user_blocks").delete().eq("user_id", req.userId).eq("blocked_id", userId));
    const { data: myDirects } = await safe(
      supabase.from("conversation_participants").select("conversation_id,conversations(type)").eq("user_id", req.userId),
      { data: [] },
    );
    const directIds = (myDirects || []).filter((row) => row.conversations?.type === "direct").map((row) => row.conversation_id);
    if (directIds.length) {
      const { data: shared } = await safe(supabase.from("conversation_participants").select("conversation_id").eq("user_id", userId).in("conversation_id", directIds), { data: [] });
      const sharedIds = (shared || []).map((row) => row.conversation_id);
      if (sharedIds.length) await safe(supabase.from("conversation_participants").update({ hidden: false, request_status: "accepted" }).eq("user_id", req.userId).in("conversation_id", sharedIds));
    }
    req.app.get("io")?.to(`user:${req.userId}`).emit("privacy:block-updated", { userId, blocked: false });
    return res.json({ ok: true, active: false });
  }

  await safe(supabase.from("user_blocks").upsert({ user_id: req.userId, blocked_id: userId }, { onConflict: "user_id,blocked_id" }));
  await Promise.all([
    removeFollow(req.userId, userId),
    removeFollow(userId, req.userId),
    safe(supabase.from("friendships").delete().or(`and(user_id.eq.${req.userId},friend_id.eq.${userId}),and(user_id.eq.${userId},friend_id.eq.${req.userId})`)),
    safe(supabase.from("favorite_users").delete().eq("user_id", req.userId).eq("target_id", userId)),
  ]);
  ...
  if (directIds.length) {
    ...
    const sharedIds = (shared || []).map((row) => row.conversation_id);
    if (sharedIds.length) {
      await safe(supabase.from("conversation_participants").update({ hidden: true, request_status: "blocked" }).eq("user_id", req.userId).in("conversation_id", sharedIds));
    }
  }

  req.app.get("io")?.to(`user:${req.userId}`).emit("privacy:block-updated", { userId, blocked: true });
  req.app.get("io")?.to(`user:${userId}`).emit("privacy:blocked-by", { userId: req.userId });
  res.json({ ok: true, active: true });
});
