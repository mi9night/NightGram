...
async function resolveReportTarget(report) {
  ...
  if (type === "message") {
    const { data } = await safeQuery(supabase.from("messages").select("id,text,sender_id,conversation_id,type,attachment_url,created_at,deleted_at").eq("id", id).maybeSingle(), null);
    return data ? { type, ...data } : null;
  }
  if (type === "channel") {
...
async function resolveReportTargetUser(report) {
  ...
  if (type === "message") {
    const { data: message } = await safeQuery(supabase.from("messages").select("sender_id").eq("id", id).maybeSingle(), null);
    if (message?.sender_id) return (await safeQuery(supabase.from("users").select("id,username").eq("id", message.sender_id).maybeSingle(), null)).data;
  }
  if (type === "channel") {
