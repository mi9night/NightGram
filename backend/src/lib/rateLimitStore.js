...
function pruneMemory(now = Date.now()) {
  if (memory.size < 10000) return;
  for (const [key, value] of memory.entries()) {
    if (!value || value.resetAt <= now) memory.delete(key);
  }
  // Hard ceiling protects a long-running Railway process from unbounded IP-key growth.
  if (memory.size > 20000) {
    const overflow = memory.size - 15000;
    let removed = 0;
