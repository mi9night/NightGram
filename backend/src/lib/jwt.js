...
function signAccessToken(user, sessionId = null) {
  return jwt.sign(
    { sub: user.id, username: user.username, role: user.role, ...(sessionId ? { sid: sessionId } : {}) },
    getSecret("JWT_SECRET"),
    { expiresIn: process.env.ACCESS_TOKEN_TTL || "15m" },
  );
}

function signRefreshToken(user, sessionId = null) {
  return jwt.sign(
    { sub: user.id, username: user.username, ...(sessionId ? { sid: sessionId } : {}) },
