  ...
  event.waitUntil(
    (async () => {
      ...
      await self.registration.showNotification(payload.title || "NightGram", {
        ...
        tag:
          payload.tag ||
          (isCall ? `call:${payload.callId || "incoming"}` : "nightgram"),
        renotify: isCall,
...
self.addEventListener("notificationclick", (event) => {
  ...
  const payload = {
    ...
    notificationAction: event.action || "open",
  };
  const baseUrl = payload.url || "/messages";
  const target = new URL(baseUrl, self.location.origin);

  if (payload.kind === "call" && event.action) {
    target.searchParams.set(
      "callAction",
      event.action === "reject-call" ? "reject" : "accept",
    );
  }
    ...
    (async () => {
      ...
      for (const client of windows) {
        if ("focus" in client) {
          ...
          await client.focus();
          client.postMessage({
            type: "nightgram:notification-click",
            payload,
          });
          return;
        }
...
self.addEventListener("message", (event) => {
  ...
  if (event.data?.type === "SHOW_NOTIFICATION") {
    ...
    event.waitUntil(
      self.registration.showNotification(payload.title || "NightGram", {
        body: payload.body || "Новое событие",
